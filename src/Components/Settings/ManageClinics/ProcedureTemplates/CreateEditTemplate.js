...
class CreateEditTemplate extends Component {
    constructor(props) {
        ...
        stateVal.questionsList = [JSON.parse(JSON.stringify(defaultItem))];
        ...
        stateVal.activeBox = JSON.parse(JSON.stringify(defaultItem));
        stateVal.viewActiveBox = JSON.parse(JSON.stringify(defaultItem));
    ...
    handleChildChange = (name, value) => {
      ...
      if(value) {
        if( actualName == 'questions' || actualName == 'answers') {
          ...
          oldVal[nameArr[2]] = parseInt(value);
    ...
    changeBoxType = event => {
      ...
      if(type == 'single' || type == 'scale' || type == 'yesno') {
        ...
        returnState['answersError'][activeBox.obj.id] = false;
      }
    ...
    addQuestion = event => {
      let questionsError = this.state.questionsError; 
      let questionsList = JSON.parse(JSON.stringify(this.state.questionsList));
      let listLength = questionsList.length;
      let newQuestion = JSON.parse(JSON.stringify(defaultItem));
      newQuestion.displayOrder = listLength+1;
      newQuestion.id = listLength+1;
      questionsError[listLength+1] = [false]
      newQuestion.currentlyActive = true;
      questionsList.push(newQuestion);

      let activeBox = this.getActiveBox();
      questionsList[activeBox.index].currentlyActive = false;

      this.setState({questionsList: questionsList, questionsError:questionsError}, () => {
        let options = this.getAllQuestionsNameWithId();
        this.setState({mainOptions: options});
      });
    }
    ...
    // delete addLogicQuestion
    deleteLogicJump = (lid) => {
      let activeBox = this.getActiveBox();
      let id = activeBox.obj.id
      let questions = JSON.parse(JSON.stringify(this.state["questions-"+id]));
      let answers = JSON.parse(JSON.stringify(this.state["answers-"+id]));
      let qList = this.state.questionsList;
      /*if(questions.length == 1) {
        return false;
      }*/
      
      if(answers[lid]) {
        answers.splice(lid, 1);  
      }

      questions.splice(lid, 1);
      let returnState = {};
      let questionIndex = qList.findIndex(e => (e.id == id));
      let qId = questionIndex;
      console.log(qId, qList);
      returnState["questions-"+id] = questions;
      returnState["answers-"+id] = answers;
      qList[qId].logicJumpsQue = answers;
      qList[qId].logicJumpsAns = questions;
    ...
    makeActiveBox = (qid, event) => {
      ...
      this.setState({questionsList: this.state.questionsList, activeBox: this.state.questionsList[id], viewActiveBox: this.state.questionsList[id], activeBoxRequired: this.state.questionsList[id].required, activeBoxDesc: this.state.questionsList[id].description, userChanged: true, activeBoxTypeLabel: boxTypeArr[this.state.questionsList[id].boxtype]});
    ...
    handleKeyUp = event => {
      ...
      if (event.keyCode == 8 || event == 8  ) {
        if(value == "" && options.length > 1) {
          ...
          returnState['choice-'+nameArr[1]+'-0'] = ""
          returnState['choiceError-'+nameArr[1]+'-0'] = false
          options = [""]
        }
      }
      ...
      options.map((obj, idx) => {
        if(obj != "") {
          ...
          returnState['choice-'+nameArr[1]+'-'+idx] = ""
          returnState['choiceError-'+nameArr[1]+'-'+idx] = false
        }
      })
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
      console.log(prevState.questionsList);
     ...
     if(nextProps.savedTemplateData != undefined && nextProps.savedTemplateData == true) {
      ...
      if (nextProps.templateData != undefined && nextProps.templateData != prevState.templateData && prevState.templateDataTimestamp != nextProps.templateDataTimestamp) {
        if(localStorage.getItem("showLoader") == "false") {
          ...
          returnState.questionsList = qList;
          returnState.templateDataTimestamp = nextProps.templateDataTimestamp;
      ...
      qList.map((obj, idx) => {
        ...
        if(obj.logicJumpsAns.length > obj.logicJumpsQue.length) {
          obj.logicJumpsAns.map((objInner, idxInner) => {
            errorObj.questionsError = JSON.parse(JSON.stringify(this.state.questionsError))
              ...
              content: (
                ...
                <React.Fragment key={"boxtypeYesNo-"+obj.id}>
                    ...
                    <div className="login-jump-outer">
                      <div className="logic-jump-header" data-qid={obj.id} onClick={this.toggleLogicJump}>
                          ...
                          <a className={(this.state.upDown == false) ? "pull-right" : "pull-right no-display"} name="upDown" value = {this.state.upDown}  >
                              <i className="pull-right fas no-margin loginjumponoff fa-angle-down" />
                            </a>
                            <a className={(this.state.upDown == true) ? "pull-right" : "pull-right no-display"} name="upDown" value = {this.state.upDown}  >
                     ...
                     <div  className="logic-content">
                        <div className="when-ans" data-test={typeof this.state['questions-'+ obj.id]} >
                            ...
                            <a className={this.state['questions-'+ obj.id].length <= obj.options.length-1 ? "header-select-btn line-btn add-logic-btn" : "header-select-btn line-btn add-logic-btn no-display"}  onClick={this.addLogicQuestion}>{this.state.globalLang.global_add_logic_jump}</a>
                        </div>
                ...
                <React.Fragment key={"boxtypeSingle-"+obj.id}>
                      ...
                      <div className="login-jump-outer">
                        <div className="logic-jump-header" data-qid={obj.id} onClick={this.toggleLogicJump1}>
                            <i className="fas fa-share-alt share-header-icon" />
                            <span className="logic-jump-title">{this.state.globalLang.global_logic_jump}</span>
                            <a className={(this.state.upDown == false) ? "pull-right" : "pull-right no-display"} name="upDown" value = {this.state.upDown}   >
                              <i className="pull-right fas no-margin loginjumponoff fa-angle-down" />
                            </a>
                            <a className={(this.state.upDown == true) ? "pull-right" : "pull-right no-display"} name="upDown" value = {this.state.upDown} data-qid={obj.id}  >
                              <i className="pull-right fas no-margin loginjumponoff fa-angle-up" />
                            </a>
                        </div>
                        {this.state['showLogicJump-'+obj.id] &&
                      ...
                      <div className="login-jump-outer">
                        ...
                        {this.state['showLogicJump-'+obj.id] &&
                           ...
                           <div  className="logic-content">
                              ...
                              <LogicJump 
                                qid={obj.id} 
                                answers={this.state["answers-"+obj.id]} 
                                questions={this.state['questions-'+ obj.id]} 
                                type="scale"  
                                options={obj.options} 
                                addLogicQuestion={this.addLogicQuestion} 
                                mainOptions={this.state.mainOptions} 
                                handleChildChange={this.handleChildChange} 
                                else={this.state['else-'+obj.id]} 
                                deleteLogicJump={this.deleteLogicJump} 
                                questionsError={this.state.questionsError} 
                                answersError={this.state.answersError} 
                                elseError={this.state['logicElseError-'+obj.id]}  />
      ...
      return (
        <div className="wide-popup">
          <div className="modal-blue-header front-end-popup-title">
              ...
              <span className="popup-blue-name">{this.state.globalLang.global_template} {(this.state.title && this.state.templateId) ? " - "+this.state.title : "" }</span>
          ...
          <div className="survey-outer">
              ...
              <div className={(!this.state.fullScreen) ? ((this.state.sideMenu) ? "survey-middle col-md-6 width-50" : "survey-middle col-md-6 width-40") : "survey-middle col-md-6 no-display"}>
                  <div className={(this.state.titleError) ? "edit-question-title-outer field-error" : "edit-question-title-outer"}>
                      <input className="title-type-question" id="survey_title" type="Textbox" name="title" value={this.state.title} placeholder="Type title here..."  onChange={this.handleInputChange} onClick={() => {this.setState({sideMenu: true})}}/>
                        ...
                        <div  className={(this.state.viewActiveBox.boxtype == 'Textbox') ? "report-question-outer" : "report-question-outer no-display"} >
                          ...
                          <div className={(this.state.viewActiveBox.description) ? "quest-discription" : "quest-discription no-display"} >{this.state.viewActiveBox.descriptionText}</div>
                              ...
                              {this.state.viewActiveBox.options.map((obj, idx)=>{
                                  return (
                                      <div key={'selectedAnswers-'+idx} className={(this.state["selectedAnswers-"+this.state.viewActiveBox.id+"-"+idx]) ? "multisel activated" : "multisel"}  >
                                          <input id={"selectedAnswers-"+this.state.viewActiveBox.id+"-"+idx}  className="servy-checkbox servy-radio preboxval" onChange={this.handleInputChange} name={"selectedAnswers-"+this.state.viewActiveBox.id+"-"+idx} checked={(this.state["selectedAnswers-"+this.state.viewActiveBox.id+"-"+idx]) ? "checked" : false}   type="checkbox"  /><span  className="multi-sel-alpha">{String.fromCharCode(65+idx)}</span>
                                          <label  className="multisel-label" htmlFor={"selectedAnswers-"+this.state.viewActiveBox.id+"-"+idx}>{obj}</label><i className="fa fa-check" />
                                      </div>
                                    )
                              })}
                              </div>
...
function mapStateToProps(state) {
  ...
  if (state.SettingReducer.action === "PROCEDURE_TEMPLATE_DATA") {
    if(state.SettingReducer.data.status != 200) {
      ...
      returnState.templateDataTimestamp = new Date()
    }
  }
