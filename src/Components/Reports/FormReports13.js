...
class FormReports13 extends Component {
  constructor(props) {
    super(props);
...
this.props.getReportTypes({},0);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    // if(prevState.id){
    //   returnState.dollar=returnState.AllReportTypes.report_details.variable1;
    //   returnState.date_selecter=returnState.AllReportTypes.report_details.variable2;
    //    return returnState;
    // }
    return null;
  }
    ...
    yearsArr.push({value:i, label:i})
      ...
      return (
                    <div style={{ opacity: "1" }} className="report-question-outer report-filling  fill-report no-padding" data-id={13}>
                      <div className="report-question">High spenders : Clients who have spent more than  <span className="empty-place number_class" >{this.state.dollar}</span> Dollars in  the last <span className="empty-place fill2" >{this.state.date_selecter}</span><span className="p_date"> days</span></div>
                      <div className="report-instruction">
                      {this.state.reportsLang.report_type_select}
                      </div>
                      <div className="row">
                        <input type="text" name="dollar" className="report-input input_number" value={this.state.dollar} onChange={this.handleInputChange} autoComplete="off" defaultValue="" placeholder="Dollar" />
