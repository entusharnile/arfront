...
class FormReports17 extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    let referralArr=[];
    referralArr.push({value:"", label:"Select"});
    referralArr.push({value:"fb", label:"FB"});
    referralArr.push({value:"google", label:"Google"});
    referralArr.push({value:"instagram", label:"Instagram"});
    referralArr.push({value:"yelp", label:"Yelp"});
...
handleSelectChange =(selectedReferralOptions)=>{
  this.setState({selectedReferralOptions:selectedReferralOptions})
}
  loadMore = () => {
    localStorage.setItem("sortOnly", false);
    this.setState({
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (
      ...
      returnState.ReportsDataChild = nextProps.ReportTypes;
        ...
        if(prevState.id){
                  returnState.referral_source=returnState.AllReportTypes.report_details.variable1;
                }
       return returnState;
    }
    return null;
  ...
  handleSubmit = event => {
    ...
    let referralVal=0;
    if(this.state.selectedReferralOptions != undefined){
      referralVal=this.state.selectedReferralOptions.value
    }
    let report_id='';
    let formData = {
  ...
  referralArr.push({value:"yelp", label:"Yelp"});
      ...
      return (
                    ...
                    <div style={{ opacity: "1" }} className="report-question-outer report-filling  fill-report no-padding" data-id={17}>
                      <div className="report-question">Clients who have referral source <span className="empty-place fill1" >{this.state.referral_source}</span></div>
                      ...
                      <div className="row">
                        <div className="custom-select col-sm-12 select1">
                          ...
                          {(this.state.referralOptions) && <Select
