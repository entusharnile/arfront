...
import { fetchTaxes} from '../../../Actions/Sales/salesActions.js';
...
class Taxes extends Component {
  ...
  componentDidMount(){
    ...
    this.props.fetchTaxes(formData);
  }

  static getDerivedStateFromProps(props, state) {
    if (props.TaxesList !== undefined && props.TaxesList.status === 200 && state.TaxesList != props.TaxesList.data) {
      return {
        TaxesList: props.TaxesList.data,
        sales_header: props.TaxesList.data.sales_header,
        total: props.TaxesList.data.total,
        reportDataValues: props.TaxesList.data.report_data,
        locationArray: props.TaxesList.data.clinics,
        reportHeader: props.TaxesList.data.report_header,
        grouping: props.TaxesList.data.grouping,
        showLoader : false,
      };
  ...
  deleteReminder = () => {
    ...
    this.dismissModal();
    let reminders = this.state.TaxesList;
      if(reminders.length) {
        ...
        this.setState({TaxesList : reminders});
    }
    this.props.deleteTaxesList(this.state.reminderId);
    this.dismissModal();
  }
      ...
      <div id="content">
        <div className="container-fluid content setting-wrapper">
          ...
          <div className="juvly-section">
            <SalesFilter locationArray={this.state.locationArray} handleChildSubmit={this.handleChildSubmit} dateRangePick={true} reportType={'taxes'} />
            <div className="bg-light-blue juvly-container">
              <div className="juvly-title">{this.state.salesLang.sales_taxes}</div>
...
function mapStateToProps(state) {
  ...
  if (state.SalesReducer.action === "TAXES_LIST") {
    if (state.SalesReducer.data.status === 200) {
      return {
  			TaxesList: state.SalesReducer.data
      };
    }
  }
  else {
