...
import { fetchItemSales} from '../../../Actions/Sales/salesActions.js';
...
class ItemSales extends Component {
  ...
  componentDidMount(){
    ...
    this.props.fetchItemSales(formData);
  }

  static getDerivedStateFromProps(props, state) {
    if (props.ItemSalesList !== undefined && props.ItemSalesList.status === 200 && state.ItemSalesList != props.ItemSalesList.data) {
      let graphData = [];
      if(props.ItemSalesList.data.report_data.length ){
        props.ItemSalesList.data.report_data.map((obj, idx) =>{
          if(idx< 5){
      ...
      return {
        ItemSalesList: props.ItemSalesList.data,
        sales_header: props.ItemSalesList.data.sales_header,
        total: props.ItemSalesList.data.total,
        reportDataValues: props.ItemSalesList.data.report_data,
        locationArray: props.ItemSalesList.data.clinics,
        reportHeader: props.ItemSalesList.data.report_header,
        grouping: props.ItemSalesList.data.grouping,
        showLoader : false,
        graphData: graphData
      };
      }
  ...
  deleteReminder = () => {
    ...
    this.dismissModal();
    let reminders = this.state.ItemSalesList;
      if(reminders.length) {
        ...
        this.setState({ItemSalesList : reminders});
    }
    this.props.deleteItemSalesList(this.state.reminderId);
    this.dismissModal();
  }
      ...
      <div id="content">
        <div className="container-fluid content setting-wrapper">
          ...
          <div className="juvly-section">
            <SalesFilter locationArray={this.state.locationArray} handleChildSubmit={this.handleChildSubmit} clinicVal= {this.state.selectedLocationIdList} location={true} dateRangePick={true} reportType={'item_sales'} />
            { this.state.graphData.length > 0 && <ItemGraph graphData={this.state.graphData} /> }
            <div className="bg-light-blue juvly-container">
              <div className="juvly-title">{this.state.salesLang.sales_Item_Sales}</div>
...
function mapStateToProps(state) {
  ...
  if (state.SalesReducer.action === "ITEM_SALES_LIST") {
    if (state.SalesReducer.data.status === 200) {
      return {
  			ItemSalesList: state.SalesReducer.data
      };
    }
  }
  else {
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      fetchItemSales: fetchItemSales
    }, dispatch)
}

export default connect(mapStateToProps, mapDispatchToProps)(ItemSales);
