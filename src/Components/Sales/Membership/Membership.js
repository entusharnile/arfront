...
import { fetchMembership} from '../../../Actions/Sales/salesActions.js';
...
class Membership extends Component {
  ...
  componentDidMount(){
    ...
    this.props.fetchMembership(formData);
  }
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.MembershipList !== undefined && props.MembershipList.status === 200 && state.MembershipList != props.MembershipList.data) {
      return {
        ...
        sales_header: props.MembershipList.data.sales_header,
        total_active_members: props.MembershipList.data.total_active_members,
        total: props.MembershipList.data.total,
        reportDataValues: props.MembershipList.data.report_data,
        locationArray: props.MembershipList.data.clinics,
        reportHeader: props.MembershipList.data.report_header,
        grouping: props.MembershipList.data.grouping,
        showLoader : false,
      };
  ...
  deleteReminder = () => {
    ...
    this.dismissModal();
    let reminders = this.state.MembershipList;
      if(reminders.length) {
        ...
        this.setState({MembershipList : reminders});
    }
    this.props.deleteMembershipList(this.state.reminderId);
    this.dismissModal();
  }
      ...
      <div id="content">
        <div className="container-fluid content setting-wrapper">
          ...
          <div className="juvly-section">
            <SalesFilter locationArray={this.state.locationArray} handleChildSubmit={this.handleChildSubmit} total_active_members={this.state.total_active_members} clinicVal= {this.state.selectedLocationIdList} total_active_members_value={true} dateRangePick={true} location={true} reportType={'memberships'} />
                    ...
                    this.state.reportDataValues.length > 0  && this.state.reportDataValues.map((objRes, idxRes) => {
                      return (
                        <tr className="table-updated-tr">{this.state.reportHeader.length > 0 && this.state.reportHeader.map((obj, idx) => {
                            return (
                              <td className={idx == 3 ? "col-xs-2 table-updated-th text-right" : "col-xs-2 table-updated-th text-left"}   dangerouslySetInnerHTML={{ __html: objRes[Object.keys(obj)] }}></td>
                              )
                          })}
                        </tr>
                      );
...
function mapStateToProps(state) {
  ...
  if (state.SalesReducer.action === "MEMBERSHIP_LIST") {
    if (state.SalesReducer.data.status === 200) {
      return {
  			MembershipList: state.SalesReducer.data
      };
    }
  }
  else {
