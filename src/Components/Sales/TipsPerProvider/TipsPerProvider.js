...
import { fetchTipsPerProvider} from '../../../Actions/Sales/salesActions.js';
...
class TipsPerProvider extends Component {
  ...
  componentDidMount(){
    ...
    this.props.fetchTipsPerProvider(formData);
  }

  static getDerivedStateFromProps(props, state) {
    if (props.TipsPerProviderList !== undefined && props.TipsPerProviderList.status === 200 && state.TipsPerProviderList != props.TipsPerProviderList.data) {
      return {
        TipsPerProviderList: props.TipsPerProviderList.data,
        sales_header: props.TipsPerProviderList.data.sales_header,
        total: props.TipsPerProviderList.data.total,
        reportDataValues: props.TipsPerProviderList.data.report_data,
        locationArray: props.TipsPerProviderList.data.clinics,
        reportHeader: props.TipsPerProviderList.data.report_header,
        grouping: props.TipsPerProviderList.data.grouping,
        showLoader : false,
      };
      }
      else
      ...
      <div id="content">
        <div className="container-fluid content setting-wrapper">
          ...
          <div className="juvly-section">
            <SalesFilter locationArray={this.state.locationArray} handleChildSubmit={this.handleChildSubmit} dateRangePick={true} reportType={'tip_report'} />
              ...
              <thead className="table-updated-thead">
                ...
                <tbody className="ajax_body">
                    ...
                    this.state.reportDataValues.length > 0  && this.state.reportDataValues.map((objRes, idxRes) => {
                      return (
                        <tr className="table-updated-tr" key={idxRes}>
                          {this.state.reportHeader.length > 0 && this.state.reportHeader.map((obj, idx) => {
                            return (
                              <td className={idx == 0 ? "col-xs-2 table-updated-th text-left" : "col-xs-2 table-updated-th text-right"} key={idx} >{objRes[Object.keys(obj)]}</td>
                              )
                          })}
                        </tr>
                      );
                    ...
                    <tr className="table-updated-tr">
                      <td className="col-xs-2 table-updated-td"><b>{this.state.salesLang.sales_total}</b></td>
                  {
                  ...
                  : <td className="col-xs-2 table-updated-td text-right"><b>0</b></td>
                }
                  </tr>
                </tbody>
...
function mapStateToProps(state) {
  ...
  if (state.SalesReducer.action === "FETCH_TIPS_PER_PROVIDER") {
    if (state.SalesReducer.data.status === 200) {
      return {
  			TipsPerProviderList: state.SalesReducer.data
      };
    }
  }
  else {
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      fetchTipsPerProvider: fetchTipsPerProvider
    }, dispatch)
}

export default connect(mapStateToProps, mapDispatchToProps)(TipsPerProvider);
